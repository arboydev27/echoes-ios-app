    ...
    var body: some View {
        ZStack {
            ...
            Circle()
                ...
                .blur(radius: 40)
                .scaleEffect(isAnimating ? (isRecording ? 1.1 : 0.95) : (isRecording ? 0.9 : 0.85))
                .frame(width: 300, height: 300)
            
            // Core Blob
            BlobShape(phase: phase, amplitude: isRecording ? 1.0 : 0.0)
                .fill(
            ...
            Circle()
                ...
                .scaleEffect(isAnimating ? (isRecording ? 1.05 : 0.98) : (isRecording ? 0.95 : 0.9))
                .animation(.easeInOut(duration: 0.5), value: isRecording)
        }
        .onAppear {
...
struct BlobShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        for angle in stride(from: 0, to: .pi * 2, by: 0.1) {
            ...
            let offset = (sin(angle * 3 + phase) * 15 + cos(angle * 4 + phase) * 10) * amplitude
